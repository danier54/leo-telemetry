...
class DecodedQueueReader(Protocol):
    """Tnterface for reading decoded frames across track boundaries."""
...
async def run(config: DemuxConfig | None = None) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "RedisDecodedQueue is not merged yet. Cannot boot demux daemon."
        ) from exc

    redis_client = Redis.from_url(cfg.redis_url)
    in_queue: DecodedQueueReader = RedisDecodedQueue(redis_client)
...
async def _demux_once(
    in_queue: DecodedQueueReader, out_queue: RedisTelemetryQueue
) -> bool:
