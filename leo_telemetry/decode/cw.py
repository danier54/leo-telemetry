...
# Satellites whose downlink is a CW beacon rather than AX.25 framing.
CW_NORAD_IDS: tuple[int, ...] = (31130,)

_CALLSIGN = "K5USL"
_PACKET_TYPES = frozenset("123")
_HEX_DIGITS = frozenset(string.hexdigits.upper())
# Callsign + type digit + at least one two-character hex field.
_MIN_BEACON_CHARS = len(_CALLSIGN) + 3
...
def decode_cw_beacon(raw: RawFrame) -> DecodedFrame | None:
    ...
    marker = normalized.find(_CALLSIGN)
    if marker < 0:
        return None

    beacon = normalized[marker:]
    if len(beacon) < _MIN_BEACON_CHARS:
        return None

    packet_type = beacon[len(_CALLSIGN)]
    ...
    fields = beacon[len(_CALLSIGN) + 1 :]
    hex_run = 0
    while hex_run < len(fields) and fields[hex_run] in _HEX_DIGITS:
        hex_run += 1
    if hex_run < 2:
        return None

    payload = f"{_CALLSIGN}{packet_type}{fields[:hex_run]}".encode("ascii")
    logger.debug(
